...
module.exports = async (directoryPath, remotePath, deleteFilesInBucket) => {
  s3.init();
  ...
  if (clearBucket) {
    await s3.clearBucket(process.env.BUCKET_NAME).then(clean => {
      if (fileList.length && clean) {
        async.mapLimit(fileList, LIMIT_CONCURRENT_FILES, s3.uploadFile(directoryPath, remotePath), (err, filesUploaded) => {
          if (err) {
